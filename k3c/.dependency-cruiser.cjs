  ...
  forbidden: [
    ...
    },
    {
      ...
      comment:
        ...
        'files (.d.ts), tsconfig.json and some of the babel and webpack configs. No need to do anything now. ' +
        'This can be handled in a separate issue.',
      severity: 'info',
      from: {
        orphan: true,
        pathNot: [
          ...
          '(^|/)app/routes[.]ts$', // React-Router routes config
    ...
    {
      name: 'not-to-db-unless-test-or-service',
      comment:
        'Only tests and services (i.e. modules living in app/.server may import the database stuff from app/.server/db modules. ' +
        'If you need to talk to the database, the correct pattern is to write a service that runs your queries.',
      severity: 'error',
      from: {
        path: '^(app)',
        pathNot:
          '([.](?:spec|test)[.](?:js|mjs|cjs|jsx|ts|mts|cts|tsx)$)|(app/[.]server/.+$)',
      },
      to: {
        path: 'app/[.]server/db/.+$',
      },
...
// generated: dependency-cruiser@17.3.9 on 2026-03-16T16:51:11.271Z
